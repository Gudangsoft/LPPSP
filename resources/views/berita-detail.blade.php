...
<section class="section">
    <div class="pd-single">
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:24px;">
            <span class="card-badge"><i class="fas fa-bullhorn"></i> Berita</span>
            @if($berita->tanggal_terbit)<span class="card-badge" style="background:#fff3cd;color:#856404;"><i class="fas fa-calendar"></i> {{ $berita->tanggal_terbit->translatedFormat('d F Y') }}</span>@endif
        </div>
        ...
        @endif
        <div style="line-height:1.9;color:#2d3748;font-size:1.05rem;">{!! nl2br(e($berita->konten ?? $berita->deskripsi)) !!}</div>
        @if($berita->galeri && count($berita->galeri) > 0)
        <div class="pd-gallery">
            @foreach($berita->galeri as $img)
